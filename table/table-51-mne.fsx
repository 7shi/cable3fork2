...
let mnemonic = System.IO.File.ReadAllLines "../../../mnemonic.tsv"
...
let mnemonics = mnemonic |> Array.map (fun l -> l.Split('\t').[1])
...
for i in 0 .. int <| Array.max data.[8] do
    ...
    |> Seq.toArray
    data51
    |> Seq.filter (fun (_, b) -> optypemap |> Array.exists ((=) b))
    |> Seq.map (fun (i, _) -> mnemonics.[i])
    |> Seq.distinct
    |> String.concat ","
    |> printfn "%d\t%s" i
